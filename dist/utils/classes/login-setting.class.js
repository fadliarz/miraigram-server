...
class LoginSetting {
    constructor() {
        const rawDeviceSettingsList = (0, fs_1.readFileSync)("src/utils/txt/devices.txt")
            .toString()
            .split("\n");
        const rawDeviceSettings = rawDeviceSettingsList[Math.floor(Math.random() * rawDeviceSettingsList.length)].split("|");
        const proxiesList = (0, fs_1.readFileSync)("src/utils/txt/proxies.txt")
            .toString()
            .split("\n");
        ...
        this._proxy = proxiesList[Math.floor(Math.random() * proxiesList.length)].split("\r")[0]
            .replace("{USERNAME}", process.env.SMARTPROXY_USERNAME)
            .replace("{PASSWORD}", process.env.SMARTPROXY_PASSWORD);
    }
    get deviceSettings() {
